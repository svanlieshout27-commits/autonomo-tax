...
export default function EditExpenseClient({ expense }: { expense: Expense }) {
  ...
  return (
    <div style={{ padding: '32px 36px', maxWidth: '600px' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: '16px', marginBottom: '28px' }}>
        ...
        <h1 style={{ fontSize: '22px', color: 'var(--navy)' }}>Editar gasto</h1>
      </div>
      ...
      <div className="card" style={{ padding: '28px', display: 'flex', flexDirection: 'column', gap: '20px' }}>
        ...
        <div>
          <label className="label">
            {t('expenses.notesLabel')} <span style={{ color: 'var(--muted)', fontWeight: 400 }}>{t('expenses.optional')}</span>
          </label>
          <textarea
            className="input"
      ...
      <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '12px', marginTop: '24px' }}>
        ...
        <button onClick={handleSave} className="btn-primary" disabled={saving}>
          {saving ? t('expenses.saving') : `💾 Guardar cambios`}
        </button>
      </div>
    </div>
  )
