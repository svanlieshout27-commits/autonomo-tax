...
export default function EditInvoiceClient({ invoice }: { invoice: Invoice }) {
  ...
  const [form, setForm] = useState({
    ...
    iva_rate: invoice.iva_amount > 0 ? Math.round((invoice.iva_amount / invoice.subtotal) * 100) : 21,
    irpf_rate: invoice.irpf_amount > 0 ? Math.round((invoice.irpf_amount / invoice.subtotal) * 100) : 15,
  ...
  return (
    <div style={{ padding: '32px 36px', maxWidth: '820px' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: '16px', marginBottom: '28px' }}>
        <button onClick={() => router.back()} className="btn-secondary" style={{ padding: '8px 14px', fontSize: '13px' }}>
          {t('common.back')}
        </button>
        <h1 style={{ fontSize: '22px', color: 'var(--navy)' }}>Editar factura</h1>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px' }}>
        ...
        <div className="card" style={{ padding: '24px', gridColumn: '1 / -1' }}>
          <h3 style={{ fontSize: '14px', fontWeight: 700, marginBottom: '18px', color: 'var(--navy)', textTransform: 'uppercase', letterSpacing: '0.05em' }}>
            {t('invoices.invoiceData')}
          </h3>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '16px' }}>
            ...
            <div>
              ...
              <select className="input" value={form.status} onChange={e => setForm(f => ({ ...f, status: e.target.value as 'draft' | 'sent' | 'paid' | 'overdue' | 'pending' }))}>
          ...
          {items.map((it, idx) => (
            <div key={idx} style={{ display: 'grid', gridTemplateColumns: '1fr 80px 100px 36px', gap: '10px', marginBottom: '10px', alignItems: 'center' }}>
              ...
              <button
                ...
                style={{ width: '36px', height: '36px', borderRadius: '8px', border: '1.5px solid #FCA5A5', background: '#FEF2F2', color: '#EF4444', cursor: items.length === 1 ? 'not-allowed' : 'pointer', fontSize: '16px', display: 'flex', alignItems: 'center', justifyContent: 'center', opacity: items.length === 1 ? 0.4 : 1 }}
              >
                ×
              </button>
            </div>
      ...
      <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '12px', marginTop: '24px' }}>
        ...
        <button onClick={handleSave} className="btn-primary" disabled={saving}>
          {saving ? t('invoices.saving') : `💾 Guardar cambios`}
        </button>
      </div>
    </div>
  )
